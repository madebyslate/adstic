...
| `CaseStudies` | 🟡 zakodowany | `home` | **0 KB** | Cztery kafle 2 × 2 ze zdjęciami i logotypem klienta, pod nimi reszta realizacji jako wiersze z samym tytułem. Strzałka kafla wchodzi na hover i dojeżdża po skosie w stronę, w którą pokazuje; kadr zbliża się o 4 %, tytuł unosi się o 4 px. Na dotyku strzałka jest widoczna od razu i na miejscu. Pigułka z twarzą założyciela to ten sam komponent co w `WhoWeAre` (prymityw `AvatarLink`). Zakodowany ze złożenia i wartości podanych przez klienta, nie z Figmy — brak porównania ±2 px, więc nie `done`. Otwarte: kwadrat vs 8 : 7, waga 600 w tytułach, eksport logotypów 2×, plakietka „bura" na kaflu JMSolar. Szczegóły w `CaseStudies.spec.md`. |
