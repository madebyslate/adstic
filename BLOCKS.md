...
|---|---|---|---|---|
| `Hero` | 🟡 zakodowany | `home` | **0 KB** | Zakodowany ze zrzutu i materiałów z `_inbox/`, nie z Figmy — brak porównania ±2 px, więc nie `done`. Logotypy krążą po orbicie (CSS `offset-path`, 96 s, 119 fps). Otwarte: ramka mobile, loga klientów, ikona gwiazdki, wideo tła, prędkość orbity. Szczegóły w `Hero.spec.md`. |
| `TrustedBy` | 🟡 zakodowany | `home` | **0 KB** | Opinia klienta + ściana logotypów w dwóch pasach marquee (czysty CSS, kopie liczone przy buildzie). Zakodowany ze zrzutu i wartości podanych przez klienta, nie z Figmy. Blokada: brakuje 9 z 10 logotypów (jest tylko `bura`) i pliku fontu „Google Sans Code". Szczegóły w `TrustedBy.spec.md`. |
| `WhoWeAre` | 🟡 v1 | `home` | 0 KB | Zakodowany ze zrzutu, trzy szerokości sprawdzone. Bez snapshotów Playwright i bez porównania ±2 px (brak Figmy). |
| `OurImpact` | 🟡 zakodowany | `home` | **0 KB** | Etykieta, nagłówek 48/500 i zrzut panelu na czerwonej łunie. Łuna wtapia się przez eliptyczną maskę wygaszającą kadr przed jego krawędzią (`mix-blend-mode` odrzucone — patrz P-037). Zakodowany ze złożenia, nie z Figmy — brak porównania ±2 px, więc nie `done`. Szczegóły w `OurImpact.spec.md`. |
| `CaseStudies` | 🟡 zakodowany | `home` | **0 KB** | Cztery kafle 2 × 2 ze zdjęciami i logotypem klienta, pod nimi reszta realizacji jako wiersze z samym tytułem. Strzałka kafla wchodzi na hover; na dotyku jest widoczna od razu. Pigułka z twarzą założyciela to ten sam komponent co w `WhoWeAre` (prymityw `AvatarLink`). Zakodowany ze złożenia i wartości podanych przez klienta, nie z Figmy — brak porównania ±2 px, więc nie `done`. Otwarte: kwadrat vs 8 : 7, waga 600 w tytułach, eksport logotypów 2×, plakietka „bura" na kaflu JMSolar. Szczegóły w `CaseStudies.spec.md`. |
...
| `WhyChooseUs` | 🟡 zakodowany | `home` | **0 KB** | Cztery argumenty pod blokiem „Let's talk”. Pięć kresek (górna, dolna, pionowa na środku, trzy między argumentami) to obrysy pudełek siatki, nie elementy — stykają się z definicji, bo poziome rysują dzieci tej samej kolumny, której lewą krawędzią jest pionowa. Przycisk stoi na dole lewej kolumny przez `margin-block-start: auto`, nie przez odstęp od akapitu. Ikony idą plikami (`lazy`), bo cztery rysunki to ~49 KB SVG. Zakodowany ze zrzutu i zasobów z `_inbox/images/why-choose/`, nie z Figmy — brak porównania ±2 px, więc nie `done`. Otwarte: szerokość łamania opisów po prawej, animacja kropek. Szczegóły w `WhyChooseUs.spec.md`. |
...
| `Header` | 🟡 zakodowany | wszystkie | 0 KB | Logo, nawigacja i CTA z makiety. Brak menu mobilnego (< 1024 px nawigacja ukryta) i docelowych adresów podstron. |
...
3. 🟡 **Layout** — `Header` zakodowany, bez menu mobilnego. `Footer` zakodowany
   ze zrzutu; obie kolumny menu składane na wąskich ekranach.
...
| TBT | < 200 ms | 0 ms |
| CLS | < 0,1 | 0,001 |
| JS łącznie na stronę | < 150 KB | 1,3 KB (`Contact` + `VideoReviews`, wklejone w dokument) |
| CSS | < 50 KB | 16 KB |
...
`CaseStudies` też nie ruszył ani requestów przy starcie, ani JS-u: wszystkie
12 obrazów bloku leży pod foldem i idzie `loading="lazy"`, więc nie wchodzi do
ścieżki krytycznej. CSS urósł o ~1 KB. Zgłoszeń kontrastu blok nie dołożył —
wszystkie 34 pochodzą z bloków wymienionych wyżej.

`WhyChooseUs` nie ruszył ani requestów przy starcie (14), ani JS-u: cztery ikony
leżą głęboko pod foldem i idą `loading="lazy"`. CSS urósł o ~0,1 KB
...
zachowanej proporcji pliku, przycisk dosunięty do dolnej kreski i obrót podziału
o 90° poniżej 768 px.
